...
def sample_generator(generator, z, max_batch=100000, voxel_resolution=256, voxel_origin=[0,0,0], cube_length=2.0, psi=0.7):
    ...
    sigmas = torch.sigmoid(sigmas.reshape((voxel_resolution, voxel_resolution, voxel_resolution))).cpu().numpy()
...
if __name__ == '__main__':
    ...
    option_file = osp.join(osp.dirname(opt.path), 'options.txt')
    with open(option_file, 'r') as cur_file:
        ckpt_opt_line = cur_file.readlines()[-1]
    ckpt_options = ast.literal_eval(ckpt_opt_line)

    SIREN = getattr(siren, ckpt_options['model'])
    generator = getattr(generators, ckpt_options['generator'])(SIREN, ckpt_options['latent_dim']).to(device)
    ...
    for seed in range(opt.seeds_start, opt.seeds_end):
        ...
        #     mrc.data[:] = voxel_grid
        np.save(f"{opt.output_dir}/{seed}_{opt.voxel_resolution}_voxel.npy", voxel_grid)
