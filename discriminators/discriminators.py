...
class ProgressiveEncoderDiscriminator(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.freq_phase_layer = nn.Conv2d(400, 2304 * 2, 2)
        self.img_size_to_layer = {2:8, 4:7, 8:6, 16:5, 32:4, 64:3, 128:2, 256:1, 512:0}


    def forward(self, input, alpha, instance_noise=0, pred_freq_phase=False, **kwargs):
        if instance_noise > 0:
        ...
        if pred_freq_phase:
            freq_phase = self.freq_phase_layer(x).reshape(x.shape[0], -1)
            freq, phase = torch.split(freq_phase, [2304, 2304], dim=-1)
            return prediction, latent, position, freq, phase
