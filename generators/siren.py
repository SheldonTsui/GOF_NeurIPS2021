...
class TALLSIREN(nn.Module):
    ...
    def forward(self, input, z, ray_directions, get_normal=False, **kwargs):
        frequencies, phase_shifts = self.mapping_network(z)
        return self.forward_with_frequencies_phase_shifts(input, frequencies, phase_shifts, ray_directions, get_normal=get_normal, **kwargs)

    def forward_with_frequencies_phase_shifts(self, input, frequencies, phase_shifts, ray_directions, get_normal=False, **kwargs):
        ...
        with torch.set_grad_enabled(with_grad or get_normal):
            ...
            if get_normal:
                input_grad = torch.autograd.grad(torch.sum(torch.sigmoid(sigma)), input, create_graph=with_grad)[0]
