...
def fancy_integration(rgb_sigma, z_vals, device, noise_std=0.5, last_back=False, white_back=False, clamp_mode=None, fill_mode=None, pred_occ=False):
    ...
    n = z_vals.size(2)
    depth_var = torch.sum(weights * (z_vals - depth_final.unsqueeze(2))**2, dim=2) / ((weights_sum+1e-5)*(n-1)/n) # for training observation
    ...
    integ_results = {}
    integ_results['rgb'] = rgb_final
    integ_results['depth'] = depth_final
    integ_results['depth_var'] = depth_var
    integ_results['weights'] = weights
...
def perturb_points_global(points, z_vals, ray_directions, device):
    distance_between_points = z_vals[:, 0:1, 1:2, :] - z_vals[:, 0:1, 0:1, :]
    offset = (torch.rand((z_vals.shape[0], 1, z_vals.shape[2], z_vals.shape[3]), device=device) - 0.5) * distance_between_points
...
def transform_sampled_points(points, z_vals, ray_directions, device, h_stddev=1, v_stddev=1, h_mean=math.pi * 0.5, v_mean=math.pi * 0.5, mode='normal'):
    ...
    # points, z_vals = perturb_points_global(points, z_vals, ray_directions, device)
...
def transform_sampled_points_camera_pose(points, z_vals, ray_directions, camera_origin, pitch, yaw, device):
    n, num_rays, num_steps, channels = points.shape
